...
{
    public static void DrawPlane(Plane plane, Vector3 center, Color color, float duration = 0f, float length = 10.0f)
    {
        #if UNITY_EDITOR
        ...
        Vector3 _position = center;

        DrawCross(_position, color, length / 10, duration);

        Debug.DrawLine(_position + (t1 + t2) * length, _position + (t1 - t2) * length, color, duration);
        Debug.DrawLine(_position + (-t1 + t2) * length, _position + (-t1 - t2) * length, color, duration);
        Debug.DrawLine(_position + (t1 + t2) * length, _position + (-t1 + t2) * length, color, duration);
        Debug.DrawLine(_position + (t1 - t2) * length, _position + (-t1 - t2) * length, color, duration);
        #endif
