    ...
    public static class OGTEditorGUI
    {
        ...
        public static void Button(UnityEngine.Object target, MethodInfo methodInfo)
        {
            ...
            if (methodInfo.GetParameters().All(p => p.IsOptional))
            {
                ...
                if (GUILayout.Button(buttonText, m_buttonStyle))
                {
        ...
        public static void NativeProperty_Layout(UnityEngine.Object target, PropertyInfo property)
        {
            ...
            if (value == null)
            {
                string warning = $"{ObjectNames.NicifyVariableName(property.Name)} is null. {nameof(ShowNativePropertyAttribute)} doesn't support reference types with null value";
        ...
        public static void NonSerializedField_Layout(UnityEngine.Object target, FieldInfo field)
        {
            ...
            if (value == null)
            {
                string warning = $"{ObjectNames.NicifyVariableName(field.Name)} is null. {nameof(ShowNonSerializedFieldAttribute)} doesn't support reference types with null value";
                HelpBox_Layout(warning, MessageType.Warning, context: target);
            }
