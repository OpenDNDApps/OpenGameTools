...
{
	public static void DrawPlane(Plane _plane, Vector3 _center, Color _color, float _duration = 0f, float length = 10.0f)
	{
		#if UNITY_EDITOR
		Vector3 tangent = Vector3.Dot(_plane.normal, Vector3.up) < 0.5f ? Vector3.up : Vector3.left;

		Vector3 t1 = Vector3.Cross(_plane.normal, tangent).normalized;
		Vector3 t2 = Vector3.Cross(_plane.normal, t1).normalized;

		Vector3 _position = _center;

		DrawCross(_position, _color, length / 10, _duration);

		Debug.DrawLine(_position + (t1 + t2) * length, _position + (t1 - t2) * length, _color, _duration);
		Debug.DrawLine(_position + (-t1 + t2) * length, _position + (-t1 - t2) * length, _color, _duration);
		Debug.DrawLine(_position + (t1 + t2) * length, _position + (-t1 + t2) * length, _color, _duration);
		Debug.DrawLine(_position + (t1 - t2) * length, _position + (-t1 - t2) * length, _color, _duration);
		#endif
